...
const searchRestaurants = searchInput => fetch(`${apiRestaurantBaseurl}/search?entity_id=1138&entity_type=city&start=first&sort=rating&apikey=7707b44a119207cb30d8ac5ace0d0b6d&q=${searchInput}`)
    .then(response => response.json())
...
// Function that accepts user search input as an argument. Fetch does a GET call on a URL string to get back data. Uses api base url, ticketmaster api key and the user input. Called on eventManager.js page.
const searchConcerts = (searchInput) => fetch(`${ticketmaster_base_url}events.json?apikey=${ticketmaster_key}&city=Nashville&classificationName=${searchInput}&sort=date,asc`)
